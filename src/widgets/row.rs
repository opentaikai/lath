...
/// A structural layout widget that stacks its children horizontally
/// (left to right).  Purely geometric — no visual of its own.
///
/// # Layout
///
/// * **measure** – sums each child's width plus `spacing` gaps;
///   height is the tallest child.  Clamped to incoming constraints.
/// * **arrange** – walks children left→right with a running x-offset.
...
impl Row {
    ...
    /// Appends a child to the end of the row.
    ...
    fn measure(&self, constraints: Constraints, arena: &dyn WidgetMeasure<M>) -> Size {
        ...
        for &child_id in &self.children {
            ...
            if let Some(child) = arena.get_widget(child_id) {
                let child_constraints = Constraints::loose(f32::INFINITY, constraints.max_height);
...
mod tests {
    ...
    fn row_stacks_horizontally() {
        ...
        let root = arena.spawn(Row::new().spacing(10.0).child(a).child(b));
        arena.set_root(root);
        ...
        // Widtallest child = 40.  Widths = 100 + 10 + 80 = 190.
        assert_eq!(
            root_rect.size,
            Size {
                width: 190.0,
        ...
        // b starts at x = 100 (a's width) + 10 (spacing) = 110.
        assert_eq!(b_rect.origin, Point { x: 110.0, y: 0.0 });
    }

    #[test]
    fn row_empty_returns_zero() {
        let mut arena = UiArena::<String>::new();
        let root = arena.spawn(Row::new());
        arena.set_root(root);
